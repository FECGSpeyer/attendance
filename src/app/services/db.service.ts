  ...
  async getPersonAttendances(id: number, all: boolean = false): Promise<PersonAttendance[]> {
    ...
    return data.filter((a: any) => Boolean(a.attendance)).map((att: any): PersonAttendance => {
      ...
      return {
        ...
        attended: att.status === AttendanceStatus.Present || att.status === AttendanceStatus.Late || att.status === AttendanceStatus.LateExcused || (this.tenant()?.shift_excused_as_present && att.status === AttendanceStatus.Excused && att.notes?.includes('Schichtbedingt')),
        title,
        text: attText,
        notes: att.notes,
