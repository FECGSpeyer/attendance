...
describe('Utils', () => {
  ...
  describe('getModifiedPlayersForList - lateCount', () => {
    const createMockPlayer = (id: number, instrument: number, personAttendances: any[] = [], lastSolve?: string): any => ({
      id,
      firstName: 'Test',
      lastName: `Player${id}`,
      instrument,
    ...
    it('should only count late arrivals after lastSolve', () => {
      ...
      // lastSolve is between the two attendances
      const lastSolve = new Date();
      lastSolve.setDate(lastSolve.getDate() - 1);
      lastSolve.setHours(lastSolve.getHours() - 12);

      const player = createMockPlayer(1, 1, [
        createMockPersonAttendance(1, AttendanceStatus.Late), // Before lastSolve
        createMockPersonAttendance(2, AttendanceStatus.Late), // After lastSolve
      ], lastSolve.toISOString());
      ...
      // Only the late arrival after lastSolve should be counted
      expect(result[0].lateCount).toBe(1);
    });
  ...
  describe('evaluateCriticalRules', () => {
    const createMockAttendanceMap = (attendances: any[]): Map<number, any> => {
      const map = new Map();
    ...
    const createMockTypeMap = (types: any[]): Map<string, any> => {
      const map = new Map();
    ...
    it('should return false when no rules are defined', () => {
      const result = Utils.evaluateCriticalRules([], new Map(), new Map(), []);
    ...
    it('should return false when no attendances exist', () => {
      ...
      const result = Utils.evaluateCriticalRules([], new Map(), new Map(), rules);
      expect(result).toBe(false);
    });

    it('should trigger when count threshold is reached', () => {
      const yesterday = new Date();
      yesterday.setDate(yesterday.getDate() - 1);

      const attendances = [
        { id: 1, date: yesterday.toISOString(), type_id: '1' },
        { id: 2, date: yesterday.toISOString(), type_id: '1' },
        { id: 3, date: yesterday.toISOString(), type_id: '1' },
      ];

      const personAttendances = [
        { attendance_id: 1, status: AttendanceStatus.Absent },
        { attendance_id: 2, status: AttendanceStatus.Absent },
        { attendance_id: 3, status: AttendanceStatus.Absent },
      ];
      ...
      const result = Utils.evaluateCriticalRules(
        personAttendances as any,
        createMockAttendanceMap(attendances),
        createMockTypeMap([{ id: '1' }]),
        rules
      );
    ...
    it('should not trigger when count threshold is not reached', () => {
      const yesterday = new Date();
      yesterday.setDate(yesterday.getDate() - 1);
    ...
    it('should respect lastSolve date when counting', () => {
      const twoDaysAgo = new Date();
      twoDaysAgo.setDate(twoDaysAgo.getDate() - 2);
      const yesterday = new Date();
      yesterday.setDate(yesterday.getDate() - 1);
      ...
      const attendances = [
        { id: 1, date: twoDaysAgo.toISOString(), type_id: '1' },
        { id: 2, date: twoDaysAgo.toISOString(), type_id: '1' },
        { id: 3, date: yesterday.toISOString(), type_id: '1' },
      ];
      ...
      // lastSolve is between the attendances - only 1 absence after lastSolve
      const lastSolve = new Date();
      lastSolve.setDate(lastSolve.getDate() - 1);
      lastSolve.setHours(lastSolve.getHours() - 12);
      ...
      expect(result).toBe(false); // Only 1 absence after lastSolve, threshold is 3
    });
  });
});
